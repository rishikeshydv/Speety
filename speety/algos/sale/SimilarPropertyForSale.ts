...
const SimilarPropertyForSale = async(beds:string, baths:string, price:string, zipcode:string,radius:number) => {
    ...
    buyResults.forEach((doc) => {
        ...
        keys.map((key) => {
            if (data[key].transactionType === "Sell"){
            if (Number(data[key].beds) < (Number(beds)+1) && Number(data[key].beds) > (Number(beds)-1)){
                if (Number(data[key].baths) < (Number(baths)+1) && Number(data[key].baths) > (Number(baths)-1)){
                    if (Number(data[key].price) < (Number(price)+50000) && Number(data[key].price) > (Number(price)-50000)){
...
export default SimilarPropertyForSale;